...
public class OnlineGameplayActivity extends AppCompatActivity {
    ...
    public void PauseDialog() {
        final Dialog dialog = new Dialog(OnlineGameplayActivity.this);
        ...
        main_menu.setOnClickListener(v -> {
            ...
            mp.start();
            dialog.dismiss();
            Dialog dialog1= new Dialog(OnlineGameplayActivity.this);
    ...
    Integer a[] = new Integer[49];
    game localGame = new game((Arrays.asList(a)), "black", false, "", "");

    FirebaseFirestore db = FirebaseFirestore.getInstance();
    DocumentReference multiGame;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        SharedPreferences onlinePreferences = getSharedPreferences("onlineInfo", Context.MODE_PRIVATE);

        gameID = onlinePreferences.getString("gameid", "no game");
        multiGame = db.collection("games").document(gameID);

        if(onlinePreferences.getBoolean("iswhite", false)){
            white = true;
            player_2 = preferences.getString("username", "chung");
            multiGame.update("player2", player_2);
            multiGame.get().addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
                @Override
                public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                    if(task.isSuccessful()) {
                        DocumentSnapshot doc = task.getResult();
                        player_1 = doc.get("player1").toString();
                    }
                }
            });
            //DocumentSnapshot doc = multiGame.get().getResult();
            //player_1 = doc.get("player1").toString();
        }
        else {
            white = false;
            player_1 = preferences.getString("username", "chung");
            player_2 = "Not yet joined";
        }

        localGame.setPlayer2(player_2);
        localGame.setPlayer1(player_1);
        ...
        multiGame.addSnapshotListener(new EventListener<DocumentSnapshot>() {
            @Override
            public void onEvent(@Nullable DocumentSnapshot snapshot, @Nullable FirebaseFirestoreException error) {
                if(snapshot.getMetadata().hasPendingWrites()) {
                    ...
                    player_2 = snapshot.get("player2").toString();
                    if(player_2.equals("Not yet joined")){

                    }
                    else {
                        started = true;
                        List<Integer> newBoard = (List<Integer>) snapshot.get("board");
                        for(int i = 0; i < 49; i++) {
                            board[i] = (int) newBoard.get(i);
                        }
                        ...
                        if(((Boolean) snapshot.get("Passed")) && alreadyPassed) {
                            endGame();
                        }
                        if((Boolean) snapshot.get("finished")) {
                            endGame();
                        }
                    }
                }
    ...
    public void onClick(View v){
        ...
        if((white == whiteturn) && started) {
            move(id);
            localGame.setBoard(board);
            localGame.setPassed(alreadyPassed);
        }
    ...
    public class game{
        ...
        game(List<Integer> sboard, String sturn, Boolean sfinished, String splayer1, String splayer2){
            turn = sturn;
            finished = sfinished;
            board = sboard;
            player1 = splayer1;
        ...
        public void setBoard(int[] sboard) {
            for(int i = 0; i < 49; i++) {
                board.set(i, (Integer) sboard[i]);
            }
        }
